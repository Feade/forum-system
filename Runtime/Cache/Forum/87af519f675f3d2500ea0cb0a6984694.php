...
<body>
  ...
  <?php if(is_array($circular)): foreach($circular as $key=>$cir): ?><div class="item" id="first-item">
      ...
      <div class="feed-meta">
        ...
        <span><a href="<?php echo U('Forum/Adm/deleteCircular');?>?for_num=<?php echo ($cir["for_num"]); ?>">删除</a></span>
        <span onclick="show_form('#myform_<?php echo ($cir["for_num"]); ?>')"><a>修改</a></span>

        <form id="myform_<?php echo ($cir["for_num"]); ?>" class="myForm" action="<?php echo U('Forum/Adm/modifyCircular');?>" method="post"  name="myForm-<?php echo ($cir["for_num"]); ?>" onsubmit="return check_form('myForm-<?php echo ($cir["for_num"]); ?>','for_title');" >
          <div class="modify">
            <input style="display: none;" name="for_num" value="<?php echo ($cir["for_num"]); ?>">
            <input type="text" name="for_title" value="<?php echo ($cir["for_title"]); ?>" style="width: 300px;">标题
            <textarea name="for_text" rows="8" cols="150" value="<?php echo ($cir["for_text"]); ?>" style="margin-left: 200px;margin-top: 20px;" placeholder="<?php echo ($cir["for_text"]); ?>"></textarea>
            <br>
    ...
    <form id="myform_addCircular" class="myForm" action="<?php echo U('Forum/Adm/Circular');?>" method="post" name="myForm-add" onsubmit="return check_form('myForm-add','for_title');" style="margin-top: -300px;">
