...
<body>
	<br><br><br>
	<form action="<?php echo U('Forum/Adm/searchReply');?>" method="post" class="form-horizontal" name="myForm" onsubmit="return validateForm();">
	...
	<?php if(is_array($list_post)): foreach($list_post as $key=>$it): ?><div class="item" id="first-item">
			...
			<div style="margin-left: 10%; font-size: 15px;"><a href="<?php echo U('Forum/ReplyPost/index');?>?for_num=<?php echo ($it["for_fatherid"]); ?>" title="查看主贴详情"><?php echo ($it["for_title"]); ?></a></div>
			<div class="title" >
				<span id="one" style="margin-top: -30px;">
					<a href="<?php echo U('Forum/ReplyPost/index');?>?for_num=<?php echo ($it["for_fatherid"]); ?>" title="查看主贴详情"><?php echo ($it["for_text"]); ?></a>
				</span>
			</div>
	...
	<script type="text/javascript">
		function validateForm()
		{
			...
			// if(x1.length+x2.length+x3.length+x4.length+x5.length+x6.length+x7.length+x8.length+x9.length==0)
			// {
			// 	alert("搜索内容不能为空！");
			// 	return false;
			// }
